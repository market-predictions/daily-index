...
This is a continuation run using the latest stored FX report as the strategic carry-forward base, the latest technical overlay in `output/fx_technical_overlay.json`, and the live implementation files in `output/fx_portfolio_state.json` and `output/fx_valuation_history.csv`.

The strategic regime remains **mild risk-off with a defensive USD bias**. Current live portfolio value is **99,559.41 USD**, with cash at **35,841.92 USD**, gross exposure at **63,718.04 USD**, and unrealized P&L at **-439.49 USD**. The latest portfolio valuation timestamp is **2026-03-26T23:42:52Z**. The latest technical overlay is newer than the portfolio valuation and is timestamped **2026-03-27T15:31:15Z**.

The ranking still supports **USD first, CHF second**, with **JPY retained as a smaller staged hold**, **AUD retained as a staged hold**, and **EUR / GBP kept in reduce territory**. Relative to the most recent stored FX run, the live valuation is modestly firmer and the overlay remains broadly defensive. The most notable same-cycle nuance is that **MXN** now reads technically weaker than before, while **AUD** remains only mixed rather than outright positive.

The macro backdrop continues to support defensive positioning. The Federal Reserve maintained a **3.50–3.75%** target range on 18 March 2026. The ECB deposit facility remains **2.00%**. The Bank of England maintained Bank Rate at **3.75%** and continues to frame the backdrop as vulnerable to energy and inflation persistence risks. The latest indexed RBA policy decision remains the 3 February 2026 increase to **3.85%**. The RBNZ OCR remains **2.25%**. The SNB policy rate remains **0.00%**. The BoJ continues to encourage the uncollateralized overnight call rate to remain at around **0.75%**.

## 2. Portfolio action snapshot

| Sleeve | Action | Current read |
|---|---|---|
| USD | Build | Strongest defensive anchor; supported by rates, liquidity, and broad overlay strength |
| CHF | Build on weakness | Clean second-line defensive sleeve despite only negative technical confirmation |
| JPY | Hold / stage | Haven logic intact, but still strongly negative on current technical confirmation |
| AUD | Hold / stage | Mixed, not broken, but not strong enough for aggressive adding |
| CAD | Hold | Macro support intact, but weak technical confirmation caps enthusiasm |
| MXN | Hold / stage | Carry-positive macro story, but technical confirmation has weakened |
| EUR | Reduce | Lower-rate bloc plus negative technical confirmation |
| GBP | Reduce | Some rate support, but technical picture remains strongly negative |
| NZD | Sell / avoid | Weak macro urgency and negative overlay |
| ZAR | Sell / avoid | Wrong exposure for this regime |
...
- **Commodity impulse:** Supportive for CAD in macro terms; tactically constructive for AUD only at the margin; adverse for major oil importers
...
The policy backdrop still supports a defensive USD bias. The Fed maintained the federal funds target range at **3.50–3.75%** on 18 March 2026. The ECB deposit facility remains **2.00%**. The BoE maintained Bank Rate at **3.75%**. The latest indexed RBA policy decision remains the 3 February move to **3.85%**. The RBNZ OCR remains **2.25%**. The SNB policy rate remains **0.00%**. The BoJ continues to guide the uncollateralized overnight call rate to remain at around **0.75%**. That leaves USD well supported on carry, liquidity, and risk conditions, while CHF remains the cleaner second-line defensive sleeve.
...
| Swiss defensive role with intervention risk | active / investable | CHF remains the cleaner second-line defensive sleeve because the macro case is intact, even if technical confirmation is only negative |
...
| Commodity FX recovery | active / watch | AUD remains mixed rather than clearly positive, while CAD’s near-term confirmation remains weaker than the broader macro case |
...
| EM carry under controlled volatility | weakening / watch | MXN remains macro-interesting, but the overlay has softened from mixed to negative |
...
The correct portfolio posture remains **defense first, optionality second, high-beta last**. There is still no new evidence that justifies another rebalance. USD remains the strongest core anchor. CHF remains the cleaner secondary defensive allocation than JPY on this run. JPY stays in the portfolio at reduced size. AUD remains a staged allocation, but only as a measured placeholder rather than a leadership sleeve. MXN’s technical picture has weakened, which further reinforces the case for disciplined sizing and a continued preference for USD cash and CHF over higher-beta sleeves.
...
| CHF | 11.5 | Build on weakness | Defensive diversifier with cleaner current confirmation than JPY |
| JPY | 10.5 | Hold / stage | Haven sleeve retained, but current technical confirmation is strongly negative |
| CAD | 9.3 | Hold | Oil / terms-of-trade support with weaker near-term technical confirmation |
| MXN | 7.7 | Hold / stage | Carry-positive macro story, but technical confirmation has weakened to negative |
...
| ZAR | 4.8 | Sell / avoid | High-beta EM exposure not paid enough in this regime |
...
**USD — Buy**
The Fed’s latest policy setting still favors USD on carry and liquidity, and the current overlay remains positive for the dollar through broad non-USD weakness. USD remains the highest-quality base sleeve.
...
CHF remains one of the cleanest defensive diversifiers. The current overlay is now negative rather than mixed, which tempers sizing, but the broader strategic case remains intact and still looks cleaner than JPY on a risk-adjusted basis.

**GBP — Reduce**
GBP retains some rate support, but the current technical overlay remains strong negative. The latest BoE communication still does not justify leadership status for GBP.

**AUD — Hold / stage**
The latest indexed RBA policy decision remains 3.85%, but the overlay is still mixed rather than clearly positive. AUD is cyclical and not a leadership currency in a mild risk-off world, so it still deserves only a staged allocation.
...
**MXN — Hold / stage**
MXN remains carry-positive on macro logic, but the latest overlay has now softened to negative. That keeps MXN in the portfolio, but with an even stronger case for disciplined sizing rather than aggressive adding.
...
1. **CHF** — The cleanest incremental defensive addition because the macro thesis is intact even though the overlay is no longer neutral.
...
No additional position changes were executed on this run. The live portfolio state still reflects the rebalance sourced from `weekly_fx_review_260326_09.md`, with `trades_executed = 7` on 2026-03-26.
...
| USD cash | — | Long USD cash | 2026-03-25 | 1.00000000 | 1.00000000 | 35,841.92 | 0.00 | 36.00 | Buy |
...
- Technical overlay state: available same-day; USD positive, EUR strong negative, GBP strong negative, JPY strong negative, CHF negative, AUD mixed, CAD negative, NZD negative, MXN negative, ZAR negative
- Portfolio engine state: live; last valuation date 2026-03-26 and latest portfolio-state overlay timestamp 2026-03-26T23:42:52Z
- Technical overlay as of: 2026-03-27T15:31:15Z
