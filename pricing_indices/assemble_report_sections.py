...
def main() -> None:
    ...
    _write_text(combined_path, "\n\n".join([sec4, sec11, sec16]))

    print(
        f"ASSEMBLY_BLOCKS_OK | token={token} | section4={section4_path.name} | "
        f"section11={section11_path.name} | section16={section16_path.name} | combined={combined_path.name}"
    )
